...
- It also generates a table of circular statistics for all subjects, for the two types of events
"""
...
def load_angles(subject, event_type, stage):
    path = f'../events_coupling/{subject}_{event_type}_{stage}_phase_angles.txt'
...
mean_resp_ratios = {} # dict where mean inspi/duration cycle length will be stored
pooled_N_cycles_with_events = {'sp':{},'sw':{}} # dict  where number of resp cycle with at least one event is detected inside
pooled_N_cycles_without_events = {'sp':{},'sw':{}} # dicts where number of resp cycle without any event detected inside
pooled_N_cycles_total = {'sp':{},'sw':{}} # dict where number of total resp cycle considered

for subject in subjects:
    rsp_features = pd.read_excel(f'../resp_features/{subject}_resp_features_tagged.xlsx',index_col = 0) # load resp features
    rsp_features_by_stage = rsp_features.groupby('sleep_stage').mean()  # compute mean of the inspi / duration = cycle ratio, across resp cycles, by stage
    ...
    for subrow, stage in enumerate(stages_events_select):
        ...
        mean_ratio = rsp_features_by_stage.loc[stage, 'cycle_ratio'] # sel mean respi ratio of the stage
        mean_resp_ratios[stage]  =  mean_ratio # add mean ratio to a dict

        for subcol, event_type in enumerate(event_types): # loop on events types (sp and sw) that will be displayed in columns
            phase_angles = load_angles(subject,event_type, stage) # dictionnary cycle:angles
            ...
            all_events_angles[event_type][stage] = angles_in_radian # store angles of the event type in a list which pool angles from all subjects
            ...
            pooled_N_cycles_without_events[event_type][stage] = N_cycles_without_event 
            N_cycles_total = len(phase_angles.keys()) # total number of resp cycles
            pooled_N_cycles_total[event_type][stage] = N_cycles_total
            N_cycles_with_event = N_cycles_total - N_cycles_without_event # number of cycles with at least one event detected inside
            pooled_N_cycles_with_events[event_type][stage] = N_cycles_with_event 
            ...
            if event_type == 'sp':
                row = [subject, stage,  event_types_titles[event_type] , N_events, N_cycles_with_event,N_cycles_without_event, proportion_cycles_with_event, p, stars, mu , r] # tidy row for circular stats of the subject
            else:
            ...
            ax.hist(angles_in_radian, bins = bins, density = False, edgecolor = 'black', color = bars_colors[event_type][stage]) # polar histogram of distribution of angles (in radians)
            ...
            ax.set_title(f'{stage} - {event_types_titles[event_type]} - N = {N_events}', fontsize = 15, y = 1.05) 
    ...
    for stage in stages_events_select:
        ...
        for row in range(nrows):
            for col in range(ncols):
                ...
                rsp_features = pd.read_excel(f'../resp_features/{subject}_resp_features_tagged.xlsx',index_col = 0) # load resp features
                rsp_features_by_stage = rsp_features.groupby('sleep_stage').mean()  # compute mean of the inspi / duration = cycle ratio, across resp cycles, by stage
                rsp_features['cycle_ratio'].mean() # compute mean of the inspi / duration = cycle ratio, across resp cycles
                mean_ratio = rsp_features_by_stage.loc[stage, 'cycle_ratio'] # sel mean respi ratio of the stage
                mean_resp_ratios[stage]  =  mean_ratio # add mean ratio to a dict
...
# mean_resp_ratio_by_stage = {stage:np.mean(mean_resp_ratios[stage]) for stage in stages_events_select} # mean of resp ratios across subjects
...
#     ratio_plot = mean_resp_ratio_by_stage[stage]
